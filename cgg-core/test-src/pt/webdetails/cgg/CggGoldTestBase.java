...
public class CggGoldTestBase {
  private static File checkMarkerExists( final String filename ) {
    final File file = new File( filename ); if ( file.canRead() ) {
      return file;
    } return null;
  }
  ...
  protected void run( final File file, final File gold ) throws Exception {
    final String fileNameWithoutExtension = file.getName().substring( 0, file.getName().length() - 3 );

    final String scriptType; final String outputType; final String fileName = file.getName();
    if ( fileName.endsWith( "-svg-test.js" ) ) {
      scriptType = "SVG"; outputType = "svg";
    } else if ( fileName.endsWith( "-j2d-test.js" ) ) {
      scriptType = "J2D"; outputType = "png";
    } else {
  ...
  private byte[] executeCgg( final File file, final String scriptType, final String outputType )
    ...
    cgg.draw( file.getName(), scriptType, outputType, 800, 600, parameter ); return out.toByteArray();
  }
  ...
  protected Map<String, Object> loadParameter( final File file ) throws IOException {
    ...
    final Properties p = new Properties(); final File propertiesFile = new File( properties );
    if ( propertiesFile.exists() == false ) {
      return new HashMap<String, Object>();
    }

    final FileInputStream inStream = new FileInputStream( properties ); try {
      p.load( inStream );
    } finally {
      inStream.close();
  ...
  protected void runAllGoldCharts() throws Exception {
    initializeTestEnvironment(); processCharts( "cgg" );
  }

  private void processCharts( final String sourceDirectoryName ) throws Exception {
    final File marker = findMarker(); final File cggCharts = new File( marker, sourceDirectoryName );
    final File gold = new File( marker, "gold" ); final FilenameFilter filter = createChartFilter();

    final Map<File, File> files = findFilesToProcess( cggCharts, gold, filter );

    for ( final Map.Entry<File, File> testSpec : files.entrySet() ) {
      final File file = testSpec.getKey(); final File goldTargetDirectory = testSpec.getValue();
      if ( file.isDirectory() ) {
        continue;
      }

      try {
        System.out.printf( "Processing %s%n", file ); run( file, goldTargetDirectory );
        System.out.printf( "Finished   %s%n", file );
      } catch ( Throwable re ) {
        System.out.printf( re.getMessage() ); re.printStackTrace( System.out );
        throw new Exception( "Failed at " + file, re );
      }
    }
  }

  private static Map<File, File> findFilesToProcess( final File cggCharts, final File goldDirectory,
      final FilenameFilter filter ) {
    final Map<File, File> resultFiles = new HashMap<File, File>();

    final File[] files = cggCharts.listFiles( filter ); final HashSet<String> fileSet = new HashSet<String>();
    for ( final File file : files ) {
      final String fileName = file.getName(); if ( fileSet.add( fileName.toLowerCase( Locale.ENGLISH ) ) == false ) {
  ...
  protected void runSingleGoldChart( final String file ) throws Exception {
    ...
    final File marker = findMarker(); final File gold = new File( marker, "gold" );

    try {
      final File cggChartFile = findChart( file );

      System.out.printf( "Processing %s%n", file ); run( cggChartFile, gold );
      System.out.printf( "Finished   %s%n", file );
    } catch ( Throwable re ) {
      throw new Exception( "Failed at " + file, re );
    }
  ...
  private File findChart( final String file ) throws FileNotFoundException {
    final File marker = findMarker(); final File cggCharts = new File( marker, "cgg" );
    final File cggChartFile = new File( cggCharts, file ); if ( cggChartFile.exists() ) {
      return cggChartFile;
    }
  ...
  protected void handleContent( final byte[] cggChartOutput, final File goldSample, final String outputType )
      throws Exception {
    ...
    if ( "svg".equals( outputType ) ) {
      ...
      final Reader cggChart = new InputStreamReader( inputStream, CharsetHelper.getEncoding() ); try {
    ...
    final FileInputStream in = new FileInputStream( goldSample ); try {
      final ByteArrayOutputStream out = new ByteArrayOutputStream(); IOUtils.copy( in, out );
  ...
  public static File findMarker() {
    final ArrayList<String> positions = new ArrayList<String>(); positions.add( "test-gold/marker.properties" );
    for ( final String pos : positions ) {
      final File file = checkMarkerExists( pos ); if ( file != null ) {
