...
public class JsonExporter implements Exporter {
  ...
  public String export( final TableModel tableModel ) throws IOException {
    final StringWriter stringWriter = new StringWriter(); final JsonWriter writer = new JsonWriter( stringWriter );
    writer.beginObject(); writer.name( "metadata" ); writer.beginArray();

    final int columnCount = tableModel.getColumnCount(); final int rowCount = tableModel.getRowCount();

    for ( int i = 0; i < columnCount; i++ ) {
      writer.beginObject(); writer.name( "colIndex" ).value( i );
      writer.name( "colName" ).value( tableModel.getColumnName( i ) );
      writer.name( "colType" ).value( getColType( tableModel.getColumnClass( i ) ) ); writer.endObject();
    } writer.endArray();

    writer.name( "resultset" ); writer.beginArray(); for ( int rowIdx = 0; rowIdx < rowCount; rowIdx++ ) {
      writer.beginArray(); for ( int colIdx = 0; colIdx < columnCount; colIdx++ ) {
        final Object valueAt = tableModel.getValueAt( rowIdx, colIdx ); if ( valueAt == null ) {
          writer.nullValue();
        } else {
          writer.value( String.valueOf( valueAt ) );
        }
      } writer.endArray();
    } writer.endArray(); writer.endObject(); writer.flush(); return stringWriter.toString();
  }
