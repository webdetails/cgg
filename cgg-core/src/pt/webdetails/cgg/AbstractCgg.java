...
public abstract class AbstractCgg {
  ...
  public AbstractCgg() {
    this.scriptFactory = new DefaultScriptFactory(); this.outputFactory = DefaultOutputFactory.getInstance();
    this.dataSourceFactory = DefaultDataSourceFactory.getInstance();
  }

  public synchronized void draw( final String scriptFile, final String scriptType, final String outputType,
      final int width, final int height, final Map<String, Object> params )
      throws ScriptCreationException, FileNotFoundException, ScriptExecuteException {
    try {
      final ScriptFactory factory = getScriptFactory(); factory.enterContext();
      final Script script = factory.createScript( scriptFile, scriptType );
      script.configure( width, height, getDataSourceFactory(), factory ); final Chart chart = script.execute( params );
      produceOutput( chart, outputType ); factory.exitContext();
  ...
  protected abstract void produceOutput( final Chart chart, final String requestedOutputHandler )
