...
public class BaseScope extends ImporterTopLevel {
  ...
  public BaseScope() {
    ...
    processed = new HashMap<String, org.mozilla.javascript.Script>(); sealedStdLib = false;
  }
  ...
  public void init( final Context cx ) {
    if ( initialized == false ) {
      ...
      final String[]
          names =
          { "print", "load", "lib", "_loadSvg", "_xmlToString", "getTextLenCGG", "getTextHeightCGG", "res" };
      defineFunctionProperties( names, BaseScope.class, ScriptableObject.DONTENUM ); initialized = true;
    }
  }
  ...
  public static Object print( final Context cx, final Scriptable thisObj, final Object[] args, final Function funObj ) {
    ...
    for ( final Object arg : args ) {
      final String s = Context.toString( arg ); logger.info( s );
    } return Context.getUndefinedValue();
  }

  public static Object load( final Context cx, final Scriptable thisObj, final Object[] args, final Function funObj ) {
    ...
    try {
      final String file = arg.toString(); final BaseScope scope = (BaseScope) thisObj;
      return Context.toBoolean( scope.loadScript( cx, file ) );
    } catch ( Exception e ) {
      logger.warn( "Failed to call 'load'", e ); return Context.toBoolean( false );
    }
  }
  ...
  public boolean loadScript( Context cx, String file ) throws IOException, ScriptResourceNotFoundException {
    ...
    org.mozilla.javascript.Script s = userScripts.get( contextResourceURI ); if ( s == null ) {
      Reader contextLibraryScript = scriptFactory.getContextLibraryScript( file ); try {
        logger.info( "Compiled context reader for " + file );
        s = cx.compileReader( contextLibraryScript, file, 1, null ); userScripts.put( contextResourceURI, s );
      } finally {
  ...
  private static Object unwrapFirstArgument( final Object arg ) {
    if ( arg instanceof NativeJavaObject ) {
      final NativeJavaObject no = (NativeJavaObject) arg; return no.unwrap();
    } return arg;
  }

  public static Object _loadSvg( final Context cx, final Scriptable thisObj, final Object[] args,
      final Function funObj ) {
    final Object arg = unwrapFirstArgument( args[0] );

    if ( arg == null ) {
      return Context.toBoolean( false );
    } final String file = arg.toString(); try {
      ...
      final InputStream stream = scope.scriptFactory.getContextResource( file ); try {
        ...
        final BridgeContext ctx = new BridgeContext( userAgent, loader ); new GVTBuilder().build( ctx, doc );
        final CSSEngine eng = impl.createCSSEngine( doc, ctx ); doc.setCSSEngine( eng );
        return Context.javaToJS( doc, scope );
      } finally {
        stream.close();
      }
    } catch ( Exception e ) {
      logger.warn( "Failed to call '_loadSVG'", e ); return Context.getUndefinedValue();
    }
  }

  public static Object lib( final Context cx, final Scriptable thisObj, final Object[] args, final Function funObj ) {
    final Object arg = unwrapFirstArgument( args[0] );
    ...
    if ( arg == null ) {
      ...
      logger.warn( "Failed to call 'lib'", e ); return Context.toBoolean( false );
    }
  }
  ...
  public boolean loadSystemScript( Context cx, String file ) throws IOException, ScriptResourceNotFoundException {
    ...
    String contextResourceURI = file; org.mozilla.javascript.Script s = processed.get( contextResourceURI );
    if ( s == null ) {
      Reader script = scriptFactory.getSystemLibraryScript( file ); try {
        logger.info( "Compiled system reader for " + file ); s = cx.compileReader( script, file, 1, null );
  ...
  public static Object res( final Context cx, final Scriptable thisObj, final Object[] args, final Function funObj ) {
  ...
  public static Object _xmlToString( final Context cx, final Scriptable thisObj, final Object[] args,
      final Function funObj ) {
    final Object arg = unwrapFirstArgument( args[0] ); final Node node = (Node) arg; try {
      final Source source = new DOMSource( node ); final StringWriter stringWriter = new StringWriter();
  ...
  public static Object getTextLenCGG( Context cx, Scriptable thisObj, Object[] args, Function funObj ) {
    String text = Context.toString( args[0] ); String fontFamily = Context.toString( args[1] );
    String fontSize = Context.toString( args[2] ).trim(); String fontStyle = "normal"; String fontWeight = "normal";
  ...
  public static Object getTextHeightCGG( Context cx, Scriptable thisObj, Object[] args, Function funObj ) {
    ...
    String fontFamily = Context.toString( args[1] ); String fontSize = Context.toString( args[2] ).trim();
    String fontStyle = "normal"; String fontWeight = "normal";
  ...
  private static Font getFont( String fontFamily, String fontSize, String fontStyle, String fontWeight ) {
    // Get size unit
    boolean convert = false; if ( fontSize.endsWith( "px" ) ) {
      //          convert = true;
    ...
    if ( convert ) {//px->pt
      ...
      int screenDpi = Toolkit.getDefaultToolkit().getScreenResolution(); isize = Math.round( size * screenDpi / 72.0f );
    }
  ...
  private static int parseCssFontStyleAndWeight( String fontStyle, String fontWeight ) {
    ...
    //Font.BOLD
    //Font.PLAIN
    boolean isItalic = false; boolean isBold = false;
  ...
  private static Font decodeFont( String fontFamily, int fontStyle, int isize ) {
    String fontStyleText = ""; switch ( fontStyle ) {
      case Font.BOLD: fontStyleText = "BOLD "; break;

      case Font.ITALIC: fontStyleText = "ITALIC "; break;

      case ( Font.ITALIC | Font.BOLD ): fontStyleText = "BOLDITALIC "; break;
    }
    ...
    if ( ffont.getFamily().equals( Font.DIALOG ) && !fontFamily.equals( "dialog" ) ) {
      ...
      GVTFontFamily awtFamily = FontFamilyResolver.resolve( fontFamily ); if ( awtFamily == null ) {
