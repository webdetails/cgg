...
public abstract class AbstractScriptFactory implements ScriptFactory {
  ...
  protected AbstractScriptFactory() {
    contextFactory = new ContextFactory(); contexts = new HashMap<ScriptType, BaseScope>();
  }
  ...
  public Script createScript( final String path, final String scriptType )
    ...
    ScriptType st = ScriptType.valueOf( scriptType.toUpperCase( Locale.ENGLISH ) ); return createScript( path, st );
  }

  public Script createScript( final String path, final ScriptType scriptType )
      throws ScriptResourceNotFoundException, ScriptExecuteException {
    final Script script; switch ( scriptType ) {
      case SVG: script = new SvgScript( path ); break; case J2D: script = new Java2DScript( path ); break;
      default: throw new IllegalArgumentException();
    } script.setScope( getScope( scriptType ) ); return script;
  }

  public void enterContext() {
    Context cx = contextFactory.enterContext();

    cx.setGeneratingDebug( false ); cx.setOptimizationLevel( -1 );
  }
  ...
  protected BaseScope getScope( final ScriptType type ) throws ScriptExecuteException {
    BaseScope o = contexts.get( type ); if ( o != null ) {
      return o;
    }

    o = createScope( type ); contexts.put( type, o ); return o;
  }
  ...
  protected BaseScope createScope( final ScriptType type ) throws ScriptExecuteException {
    ...
    final Context cx = contextFactory.enterContext(); cx.setGeneratingDebug( false ); cx.setOptimizationLevel( -1 );

    final BaseScope scope = new BaseScope(); scope.setScriptFactory( this ); scope.init( cx );
