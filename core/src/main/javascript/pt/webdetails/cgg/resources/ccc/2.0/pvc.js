...
define([ "./def", "./protovis-compat!", "./cdo", "jquery", "./tipsy" ], function(def, pv, cdo, $) {
    ...
    def.type("pvc.visual.Role").init(function(chart, name, keyArgs) {
        ...
        getBoundDimensionName: function() {
    ...
    def.type("pvc.CartesianAbstractPanel", pvc.PlotPanel).init(function(chart, parent, plot, options) {
        ...
        _calcLayout: function(layoutInfo) {
            ...
            if (clientSizeInfo) {
                ...
                clientSize.width = null != clientSizeFix.width ? clientSizeFix.width : Math.max(Math.min(clientSize.width, clientSizeMax.width), clientSizeMin.width);
                clientSize.height = null != clientSizeFix.height ? clientSizeFix.height : Math.max(Math.min(clientSize.height, clientSizeMax.height), clientSizeMin.height);
            }
    ...
    def.type("pvc.CategoricalAbstract", pvc.CartesianAbstract).add({
        _onWillCreatePlotPanelScene: function() {}
    });
