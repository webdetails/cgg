...
public class CdwFileNavigator extends PentahoBase {
    ...
    protected static final Log logger = LogFactory.getLog(CdwFileNavigator.class);
    ...
    public CdwFileNavigator(final IPentahoSession userSession, String contextPath) {
    ...
    public String getCdwFilelist(final String mode, final String solution, final String path) throws JSONException, ParserConfigurationException {
    ...
    private JSONArray processTree(final Node tree, final String parentPath) {
        ...
        processTree(tree, parentPath, flat);
        return flat;
    }

    private JSONArray processTree(final Node tree, final String parentPath, JSONArray flat) {
            ...
            for (final Object node1 : nodes) {
                ...
                if (parentPathArray.length > 0) {
                    ...
                    if (visible && isDirectory) {
                        ...
                        final List fileNodes = node.selectNodes("./file[@isDirectory='false'][ends-with(string(@name),'.cdw')]");
        ...
        try {
            ...
            if (!nodes.isEmpty() && nodes.size() == 1) {
                ...
                for (final Object node1 : nodes) {
                    ...
                    final Node childNode = (Node) node1;
                    final String name = childNode.valueOf("@name");
                    final String localizedName = childNode.valueOf("@localized-name");
                    final String description = childNode.valueOf("@description");
                    final String type = childNode.valueOf("@isDirectory").equals("true") ? TYPE_DIR : name.endsWith(".xaction") ? TYPE_XACTION : name.endsWith(".url") ? TYPE_URL : name.endsWith(".xcdf") ? TYPE_XCDF : name.endsWith(".wcdf") ? TYPE_WCDF : name.endsWith(".prpt") ? TYPE_PRPT : null;
                    final boolean visible = childNode.valueOf("@visible").equals("true");

                    if (type != null) {

                        /* Get the hashTable that contains the pairs:  supported-file-type -> associated url to use */
                        final Map<String, String> readAbility = new HashMap<String, String>();
                        readAbility.put("cdw", "drawCdw?path={solution}/{path}/{name}");
                        String link = "";
                        ...
                        final String url = (type != null && type.equals(TYPE_URL)) ? (!childNode.valueOf("@url").startsWith("http") && !childNode.valueOf("@url").startsWith(relativeUrl) && !childNode.valueOf("@url").startsWith("/") ? /*CdfContentGenerator.BASE_URL +*/ "/" + childNode.valueOf("@url") : childNode.valueOf("@url")) : null;
                        ...
                        json.put("description", description);
                        json.put("link", link);
                        if (type.equals(TYPE_PRPT)) {
                            json.put("prpt", name);
                        }
                        if (type.equals(TYPE_XCDF) || type.equals(TYPE_WCDF)) {
                            json.put("file", name);
                        }
                        if (type.equals(TYPE_XACTION)) {
